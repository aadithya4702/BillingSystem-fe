  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gray-900 text-white">
      ...
      <main className="flex-1 pb-6 pl-6 pr-6 mb-10 overflow-auto">
        ...
        <div className="grid grid-cols-1 relative sm:grid-cols-2 lg:grid-cols-3 md:grid-cols-1 gap-6">
      ...
      {/* Checkout Sidebar */}
      <aside
        className={`fixed bottom-0 left-0 w-full md:w-1/3 lg:w-1/4 bg-gray-800 p-4 transition-all duration-300 md:relative md:h-screen md:overflow-auto  flex flex-col ${
          showCart ? "h-1/2" : "h-[50px]"
        }`}
      >
        ...
        {showCart && (
          <div className="mt-4 space-y-4 overflow-y-auto max-h-[60%]">
            {cart.length > 0 ? (
              cart.map((item) => (
                <div
                  key={item.id}
